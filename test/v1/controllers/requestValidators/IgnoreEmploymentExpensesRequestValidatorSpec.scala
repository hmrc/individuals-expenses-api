...
class IgnoreEmploymentExpensesRequestValidatorSpec extends UnitSpec {
  ...
  private val rawData = IgnoreEmploymentExpensesRawData(validNino, validTaxYear)

  "parseRequest" should {
    "return a request object" when {
      "valid request data is supplied" in new Test {
        private val parsedRequest = IgnoreEmploymentExpensesRequest(Nino(validNino), TaxYear.fromMtd(validTaxYear))

        requestValidator.parseRequest(rawData) shouldBe Right(parsedRequest)
      }

      "a taxYear that has not ended is supplied but temporal validation is disabled" in new Test {
        private val parsedRequest = IgnoreEmploymentExpensesRequest(Nino(validNino), TaxYear.fromMtd(validTaxYear))

        requestValidator.parseRequest(rawData.copy(temporalValidationEnabled = false)) shouldBe Right(parsedRequest)
      }
    }
    ...
    "return a RuleTaxYearNotEndedError in an ErrorWrapper" when {
      "a tax year that has not ended is supplied" in new Test {
  ...
  private trait Test extends MockCurrentDateTime with MockCurrentTaxYear with MockAppConfig {
    ...
    val requestValidator = new IgnoreEmploymentExpensesRequestValidator(mockAppConfig)
