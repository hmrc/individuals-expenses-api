...
class DeleteOtherExpensesRequestValidatorSpec extends UnitSpec {
  ...
  private val rawData = DeleteOtherExpensesRawData(validNino, validTaxYear)

  "parseRequest" should {
    "return a request object" when {
      "valid request data is supplied" in new Test {
        private val parsedRequest = DeleteOtherExpensesRequest(Nino(validNino), TaxYear.fromMtd(validTaxYear))
  ...
  private trait Test extends MockCurrentDateTime with MockCurrentTaxYear with MockAppConfig {
    ...
    val requestValidator = new DeleteOtherExpensesRequestValidator(mockAppConfig)
